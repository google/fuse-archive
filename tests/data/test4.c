...
// gcc -o test -Wall test4.c -larchive && ./test
int main() {
  ...
  printf(">>> archive_read_append_filter_program()\n");
  r = archive_read_append_filter_program(a, "gzip -d");
  printf("<<< archive_read_append_filter_program() returned %d\n\n", r);
  assert(r == ARCHIVE_OK);
  ...
  const char* const data = "H4sICN9tumcAA2hpLnR4dC5icgBT9GBgcU7MU6jML1UoSk1MUSjJSCyx52IGACpOqysYAAAA";
