...
#undef SET_FORMAT
  ...
  void SetFormat() {
    ...
    // Does this extension signal a recognized archive format that is not a ZIP?
    if (SetFormat(ext)) {
      return;
    }
    ...
    // Special case for ZIP. We only want to decode ZIP archives if they are not
    // behind a filter that prevent them from being accessed in a seekable way.
    if (SetZipFormat(ext)) {
      return;
    }
